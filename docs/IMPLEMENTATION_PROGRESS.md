...
- Brand Sponsor static-data cleanup:
  - Removed `BrandSponsorDemoData`/`BrandSponsorDemoStore` usage from BR-01 dashboard, BR-02 profile, BR-03 opportunity composer, BR-04 applications inbox, BR-05 negotiation terms, BR-06 campaign tracker, BR-07 payments records, shared live helpers, shell, and components.
  - Replaced unauthenticated/demo preview branches with explicit sign-in/live-backend empty states.
  - Kept live create/update flows connected to Specialist, Auth, and Payments controllers.
  - Moved brand status label mapping into `brand_sponsor_live.dart`.
  - Deleted `lib/features/brand_sponsors/data/brand_sponsor_demo_data.dart`.
  - Feature static search is clean: no `BrandSponsorDemoData`, `BrandSponsorDemoStore`, `brand_sponsor_demo_data`, `_buildPreview`, `Preview`, or `preview` runtime matches under `lib/features/brand_sponsors`.
  - `flutter analyze` passed.
...
### 2026-09-10 — Shared visual marketplace and additive portal media

- Goal: replace the card-wall presentation with a reusable image-led marketplace, expose the same experience in Brand and Director portals, and populate existing demo records with real media without replacing the production database.
- Flutter: added one shared marketplace card/results composition with a category-coloured side rail, featured-result hierarchy, responsive supporting grid, live trust/rate/city/tag fields, and existing backend-backed actions. Brand and Director discovery now use the shared component; Director discovery also exposes the live Crew and Distribution feeds.
- Profile presentation: Brand provider detail now uses the large shared profile showcase for every provider category and renders every available media item. Shared portal section cards use a single semantic side accent, and empty image states use a branded layered fallback instead of a grey rectangle.
- Additive data: expanded `seed_brand_portal_demo.py` to install optimized public media for the Nova Cola brand/project and every public demo actor, model, influencer, crew, location, equipment, casting-agency, and distribution provider; update missing listing/profile cover pointers; and add rerun-safe provider listings where a demo profile did not have one. Real user records are not modified, and no tables, databases, or existing rows are deleted.
- Web delivery: updated the source-controlled Nginx vhost to serve `/var/www/cineconnect/web` directly, preserve `/api/` and `/media/` routing, disable inherited rate limiting, enable compression, use safe Flutter entry-point cache rules, and retain SPA fallback routing.
- Verification: `flutter analyze --no-pub` passes; all 740 Flutter tests pass; backend unit tests pass 24/24; Ruff, formatting, and mypy pass tree-wide; the expanded media seed creates 60 DB-backed image assets and 58 missing listing covers against a full local demo dataset, then completes an idempotent second run; `flutter build web --release --dart-define=CINECONNECT_API_BASE_URL=https://cine.nalexustechnologies.com/api/v1` passes.
- Production deployment: synced the release web build to `/var/www/cineconnect/web`, synced the additive media/seed into the existing release and API container, and ran it against the existing MySQL database. The first expanded production run added 60 missing public image records and 60 listing covers; the next run found all 86 demo listing media relationships already present and created no duplicates. No migration, database replacement, or real-user media update was performed.
- Nginx: installed the source-controlled vhost after preserving the host-managed Nalexus agent routes, validated it with `nginx -t`, and reloaded Nginx. The `/media/` alias uses a `^~` prefix so database-backed JPEG/PNG requests cannot be captured by the generic static-asset regex.
- Live verification: API readiness reports MySQL and Redis `ok`; the compressed `main.dart.js` now transfers externally in 1.09 seconds and CanvasKit in 1.32 seconds, compared with 109 seconds and a 120-second timeout before the vhost correction. An isolated browser confirmed the HTML boot loader disappears, Flutter mounts, and `#/login` renders; a second clean flow logged in as the Brand demo and opened `#/brand/discover`. All requested media returned HTTP 200 with no browser network failures. Evidence: `docs/server_live/cineconnect-live-verified-20260910.png` and `docs/server_live/cineconnect-brand-marketplace-live-20260910.png`.
- Follow-up cache correction: the initial vhost still allowed browsers to retain the stable `main.dart.js` filename for 24 hours. The app shell now appends a deployment-specific release token, while Nginx serves `main.dart.js` and `flutter_bootstrap.js` with `no-cache, no-store, must-revalidate`. This forces existing Safari/Chrome installations to request the picture-card release instead of reusing the earlier grey-card bundle.
- Follow-up production proof: a brand-new isolated Chrome profile signed in as the Brand demo, opened `#/brand/discover`, rendered the featured Ayaan Malik portrait and the new image/body split card with its category-coloured side rail, and downloaded all 86 database-linked marketplace media URLs. Evidence: `docs/server_live/cineconnect-brand-marketplace-fresh-20260910.png`.
- Safari media correction: public media URLs now include a content-checksum query version. This prevents an earlier cached 404/missing response for a stable `/media/...` path from hiding a file that is now present. The production Active brief returns `nova-cola-winter-stories.jpg?v=ccee711da4ed`, and that versioned JPEG returns HTTP 200. A clean production login renders the campaign photograph inside the Active brief card; evidence: `docs/server_live/cineconnect-brand-dashboard-campaign-live-20260910.png`.

### 2026-09-12 — Image-led specialist workspaces and production completion

- Specialist portals: Model, Location Owner, Crew, and Media & Equipment dashboards now use a shared image-led workspace hero with a large backend-sourced cover, profile identity, trust/readiness, status, and context actions. Location properties fall back to the owner profile cover when listing media is absent, and Model campaign media uses the live profile cover instead of a placeholder.
- Crew completion: dashboard, availability, contracts/payments, and ratings/work-history screens load real authenticated profile, booking, contract, payment, availability, and review data. The obsolete backend-gap placeholder was removed, and nullable authentication/profile states are handled without crashing.
- Shared media rendering: public marketplace, profile showcases, portal frames, Actor, Brand, Casting Agency, Crew, Distribution, Insurance, Legal, Location, Media & Equipment, and Director cards prefer browser-native image elements on web. This corrects the Safari/CanvasKit grey-image condition while keeping branded fallbacks for genuinely missing media.
- Director marketplace: the same database-backed image card system is available at `#/director/marketplace`, including categories for actors, models, influencers, crew, locations, equipment, agencies, and distribution; actions continue through the existing profile/request/project workflow. Production proof: `docs/server_live/cineconnect-director-marketplace-live-20260912.png`.
- Additive backend data: the existing production database was preserved. The demo seed now adds only missing provider cover media, portfolio items, and credit entries for Model, Crew, Location, and Equipment. A second production run created no duplicates. Equipment-provider profile loading now deterministically selects the latest matching marketplace listing when historical duplicates exist, eliminating the observed 500 response.
- Web startup: release token `20260912-specialist-v7` forces clients onto the new bundle, `main.dart.js` is immutable under its versioned URL, and Flutter obtains CanvasKit from its official CDN instead of the slow origin copy. Clean authenticated production sessions reached every specialist dashboard and the Director marketplace.
- Verification: `flutter analyze` passes; all 740 Flutter tests pass; the production release web build passes; backend lint/type checks and 26 unit tests pass. Live readiness reports MySQL and Redis `ok`, all four provider portals rendered database-backed imagery, and production API/worker/scheduler containers remained healthy. Evidence: `docs/server_live/cineconnect-model-workspace-live-20260912.png`, `cineconnect-location-workspace-live-20260912.png`, `cineconnect-crew-workspace-live-20260912.png`, and `cineconnect-equipment-workspace-live-20260912.png`.
