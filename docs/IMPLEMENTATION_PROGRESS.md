...
- Follow-up cache correction: the initial vhost still allowed browsers to retain the stable `main.dart.js` filename for 24 hours. The app shell now appends the deployment-specific `20260910-picture-cards-2` release token, while Nginx serves `main.dart.js` and `flutter_bootstrap.js` with `no-cache, no-store, must-revalidate`. This forces existing Safari/Chrome installations to request the picture-card release instead of reusing the earlier grey-card bundle.
