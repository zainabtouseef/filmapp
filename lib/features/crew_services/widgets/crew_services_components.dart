...
Color crewToneColor(BuildContext context, CrewTone tone) {
  ...
  return switch (tone) {
    CrewTone.gold => colors.goldMid,
    CrewTone.blue => colors.infoBlue,
    CrewTone.green => colors.success,
    CrewTone.purple => colors.infoPurple,
    CrewTone.danger => colors.danger,
    CrewTone.neutral => colors.textSecondary,
  };
}

Color crewStatusColor(BuildContext context, CrewBookingStatus status) {
  final colors = context.appColors;
  return switch (status) {
    ...
    CrewBookingStatus.requestReceived =>
      colors.goldMid,
    CrewBookingStatus.rejected => colors.danger,
    CrewBookingStatus.disputed => colors.danger,
    CrewBookingStatus.underNegotiation => colors.infoBlue,
  };
}

Color crewAvailabilityColor(
  BuildContext context,
  CrewAvailabilityStatus status,
) {
  final colors = context.appColors;
  return switch (status) {
    CrewAvailabilityStatus.available => colors.success,
    CrewAvailabilityStatus.unavailable => colors.textSecondary,
    CrewAvailabilityStatus.tentative => colors.goldMid,
    CrewAvailabilityStatus.booked => colors.infoBlue,
  };
}

IconData crewAvailabilityIcon(CrewAvailabilityStatus status) {
  return switch (status) {
    CrewAvailabilityStatus.available => Icons.check_circle_outline_rounded,
    CrewAvailabilityStatus.unavailable => Icons.block_rounded,
    CrewAvailabilityStatus.tentative => Icons.hourglass_top_rounded,
    CrewAvailabilityStatus.booked => Icons.event_available_outlined,
  };
}

String crewAvailabilityLabel(CrewAvailabilityStatus status) {
  return switch (status) {
...
String crewMoney(int amount) {
  return CineFormat.currency(amount);
}

void crewSnack(BuildContext context, String message) {
  showCoreSnack(context, message);
}

Future<void> showCrewSheet(
  BuildContext context, {
...
class CrewSectionCard extends StatelessWidget {
  final String title;
  final IconData icon;
  final Widget child;
  final bool selected;
...
class CrewResponsiveGrid extends StatelessWidget {
  final List<Widget> children;
  final double minWidth;
  final double gap;

  const CrewResponsiveGrid({
    super.key,
...
class CrewTwoColumn extends StatelessWidget {
  final Widget left;
  final Widget right;

  const CrewTwoColumn({super.key, required this.left, required this.right});
...
class CrewKpiRail extends StatelessWidget {
  final List<CrewMetric> metrics;

  const CrewKpiRail({super.key, required this.metrics});
      ...
      items: [
        for (final metric in metrics)
          MetricActionItem(
            ...
            accentColor: crewToneColor(context, metric.tone),
...
class CrewKpiCard extends StatelessWidget {
  final CrewMetric metric;

  const CrewKpiCard({super.key, required this.metric});
...
class CrewTaskRail extends StatelessWidget {
  final List<CrewTask> tasks;

  const CrewTaskRail({super.key, required this.tasks});
      ...
      items: [
        for (final task in tasks)
          QuickActionItem(
            ...
            tone: cineToneFromColor(context, crewToneColor(context, task.tone)),
...
class CrewMediaFrame extends StatelessWidget {
...
class _CrewFallback extends StatelessWidget {
  ...
  const _CrewFallback({required this.icon});
...
class CrewInfoRow extends StatelessWidget {
  final IconData icon;
  final String label;
  final String value;
...
class CrewTaskTile extends StatelessWidget {
  final CrewTask task;
...
class CrewStatusChip extends StatelessWidget {
  final CrewBookingStatus status;

  const CrewStatusChip({super.key, required this.status});
...
class CrewMiniBarChart extends StatelessWidget {
  final List<double> values;
  final List<Color> colors;
  final double height;

  const CrewMiniBarChart({
    super.key,
