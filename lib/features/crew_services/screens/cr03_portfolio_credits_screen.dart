...
///
/// Same 3-photo + 3-video slot pattern as the Actor/Talent, Location Owner
/// and Media/Equipment portfolio screens, backed by the same generic
/// PortfolioItem API with `profile_type: 'crew'`.
...
class _CR03PortfolioCreditsScreenState
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const TourTarget(
          id: 'crew.portfolio.pastProjects',
          child: PastRolesSection(
            profileType: 'crew',
            sectionTitle: 'Past Projects',
            emptyMessage: 'Add past projects you worked on and a cover photo.',
          ),
        ),
      ],
    );
    ...
    return FutureBuilder<List<MarketplacePortfolioItem>>(
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          ...
          children: [
            Text(
              'Add up to 3 photos and 3 videos of past work. Directors see these on your public profile.',
  ...
  Future<void> _addSlot(_SlotKind kind, int index) async {
    final controller = AuthScope.maybeOf(context);
    if (controller == null) {
      crewSnack(context, 'Sign in to upload portfolio media');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      crewSnack(context, '${kind.label} added to your portfolio');
      _reload();
    } on ApiException catch (error) {
      if (!mounted) return;
      crewSnack(context, _friendlyError(error));
    } finally {
  ...
  Future<void> _replaceSlot(
    ...
    try {
      ...
      if (!mounted) return;
      crewSnack(context, '${kind.label} replaced');
      _reload();
    } on ApiException catch (error) {
      if (!mounted) return;
      crewSnack(context, _friendlyError(error));
  ...
  Future<void> _removeSlot(
    ...
    try {
      ...
      if (!mounted) return;
      crewSnack(context, '${kind.label} removed');
      _reload();
    } on ApiException catch (error) {
      if (!mounted) return;
      crewSnack(context, _friendlyError(error));
    ...
    if (error is ApiException) {
      return switch (error.code) {
        ...
        'validation.invalid' => error.fields['file_id']?.join(' ') ??
            'Complete your service profile before adding portfolio media.',
        'network.offline' =>
